...
`prism-model-drift-loom` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for drift workflows, centered on stream reduction, windowed input fixtures, and late-data behavior checks.
...
The score is deterministic. The current threshold is 169. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 3.
